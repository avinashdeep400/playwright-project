...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'someone@example.com' }).fill('process.env.EMAIL');
  await page.getByRole('button', { name: 'Next' }).click();
  await page.getByRole('textbox', { name: 'Enter the password for' }).click();
  await page.getByRole('textbox', { name: 'Enter the password for' }).press('process.env.PASSWORD');
